      ...
      Auth.auth().currentUser?.sendEmailVerification { (error) in
        if error != nil{
          ...
          self.uploadImage(uid: myID!, image: self.image!, completionBlock: {[weak self] (fileURL, error) in
            ...
            let userCredentials = ["First_Name": self!.firstName!,
                                   "Last_Name": self!.lastName!,
                                   "Email":self!.email!,
                                   "DOB": self!.dob!,
                                   "Address": self!.address!,
                                   "Emergency_Contact_Name": self!.emergencyName!,
                                   "Emergency_Contact_Phone": self!.emergencyPhone!,
                                   "Vessel_No": self!.vesselNo!,
